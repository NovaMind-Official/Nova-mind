...
function openPayment(planName,fallbackPrice){
  ...
  const tick=setInterval(()=>{if(i>=steps.length){clearInterval(tick);setTimeout(()=>{document.getElementById("pay-loading").style.display="none";document.getElementById("pay-form").style.display="flex";updatePayNet()},500);return}fill.style.width=steps[i].p+"%";if(sub)sub.textContent=steps[i].t;i++},420);
}
...
async function verifyPayment(){
  ...
  let result;
  try{
    const r=await fetchWithTimeout(WORKER_BASE_URL+"/api/verify-payment",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({txHash:txid,plan:key})},12000);
    result=await r.json();
  }catch(e){
    btn.disabled=false;btn.classList.remove("loading");btn.textContent="✦ VERIFY PAYMENT & ACTIVATE PLAN";
    showToast("Couldn't reach the verification server — check your connection.","error");haptic("error");
